...
///
/// This is a crucial part of the release flow to prevent inconsitencies in publishing dependents of these changed crates.
/// For example:
...
pub(crate) fn apply_dev_versions<'a>(
    ...
    let mut applicable_crates = ws
        .members()?
        .into_iter()
        .filter(|crt| crt.state().changed_sinced_previous_release())
        .map(|crt| (crt.name(), *crt))
        .collect::<HashMap<_, _>>();

    let mut queue = applicable_crates
        .values()
        .map(|crt| *crt)
        .collect::<Vec<_>>();
    ...
    while let Some(crt) = queue.pop() {
        ...
        for changed_dependant in crate::common::set_version(dry_run, crt, &version)? {
            if applicable_crates
                ...
                .is_none()
            {
                if changed_dependant.state().has_previous_release() {
    ...
    if !msg.is_empty() {
        let commit_msg = indoc::formatdoc! {r#"
            release-automation: applying develop versions to changed crates
